...
**YesBoss** 是什么？
- 🤖 **多Agent协作系统**: Master负责任务规划，Workers负责任务执行
- 💬 **深度IM集成**: 在飞书群中直接@Bot发送任务，无需切换应用
- 🔧 **自动化工具调用**: 自动调用Bash命令、文件操作、Web搜索等工具
- 🛡️ **安全沙箱保护**: 高危操作自动拦截，需要人工批准才能执行
- 📊 **实时进度反馈**: 在群聊中实时查看任务执行进度
- 📝 **自动生成报告**: 任务完成后自动生成详细的总结报告
...
4. **如何使用Bot**
   - **重要**: 使用 `@应用名称` 来提及Bot
   - 例如：如果你的应用名称是 "YesBot"，则输入：
     ```
     @YesBot 分析当前项目的代码结构
     ```
   - 如果应用名称包含中文或特殊字符，确保正确输入

5. **查看应用名称**
   - 在飞书开放平台的应用管理页面可以看到应用名称
   - 或在群聊中查看机器人的名称
...
4. 在应用管理 → 功能权限 → 事件订阅：
   ...
   - 请求URL: `https://your-domain.com/webhook/feishu`
   - 订阅事件: `im.message.receive_v1`
5. 启用权限：`im:message`、`im:message:group_at_msg`、`im:chat`

#### 步骤3️⃣: 本地开发配置（使用ngrok）
```bash
# 1. 启动ngrok隧道
ngrok http 6000

# 2. 复制输出的HTTPS地址
# 示例：https://abcd1234.ngrok.io

# 3. 在飞书开放平台更新Webhook URL
```
...
2. 推送审批卡片到群聊，显示：
   ```
   ⚠️ 高危操作拦截 - 删除文件
   被拦截的命令: rm -rf /path/to/file

   该操作存在安全风险，需要您手动授权后才能继续执行。
   [✓ 批准执行]  [✗ 拒绝执行]
   ```
3. 你点击"批准"或"拒绝"按钮
4. 系统根据你的决定继续或终止
...
**好的任务描述**：
```
@YesBot 分析src/main/java目录下的代码，生成API文档
```
...
# 3. 检查网络连接
ping api.open.bigmodel.cn
```

#### 4. 如何重新运行任务？

在飞书群中重新发送相同的任务描述即可，系统会创建新的执行会话。

#### 5. 如何中断任务？

目前版本暂不支持主动中断任务。如需紧急停止：
```bash
