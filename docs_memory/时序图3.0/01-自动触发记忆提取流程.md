...
sequenceDiagram
    ...
    alt 满足触发条件
        ...
        loop 对每个segment
            ...
            Note over ContentProcessor: LLM为该segment生成1-2句abstract摘要<br/>详见《03-ResourceCaption生成详细流程.md》
...
本文档（01）是架构视角文档，关注完整流程和接口调用关系。

**步骤3详细展开**：
- 本文档展示：`ContentProcessor::generateSegmentAbstract(segmentContent)` 的调用时机和位置
- 详细实现见：《03-ResourceCaption生成详细流程.md》
- 03文档包含：
  - LLM调用参数配置（model、temperature、maxTokens等）
  - Prompt工程策略（模板设计、Few-shot学习）
  - 批量处理优化（策略选择、并发控制）
  - 质量保证机制（验证器、指标监控）
  - 缓存和降级策略

**文档关系**：
- 01文档（本文档）：架构视角 - "何时调用"、"调用关系"
- 03文档：实现视角 - "如何实现"、"参数配置"
